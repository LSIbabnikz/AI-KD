...
def construct_full_model(config_loc: str) -> Tuple[torch.nn.Module, Compose]:
    ...
    config = parse_config_file(config_loc)
    ##print(args_to_dict(config, {}))
    #exit()

    module_name, function_name = config.model.features.module.rsplit(".", 1)
    module = importlib.import_module(module_name)
    feature_extractor_function = getattr(module, function_name)
    feature_extractor = feature_extractor_function()

    module_name, function_name = config.model.head.module.rsplit(".", 1)
    module = importlib.import_module(module_name)
    quality_head_function = getattr(module, function_name)
    quality_head = quality_head_function()

    trans = construct_transformation(config.model.transform)

    module_name, function_name = config.model.wrapper.module.rsplit(".", 1)
    module = importlib.import_module(module_name)
    aikd_model_function = getattr(module, function_name)
    aikd_model = aikd_model_function(
        feature_extractor = feature_extractor,
        quality_head = quality_head
        )

    return aikd_model, feature_extractor, trans
...
def base_lambdalr(step, warmup_end): 
    """_summary_

    Args:
        step (_type_): _description_
        warmup_end (_type_): _description_

    Returns:
        _type_: _description_
    """
    if step < warmup_end:
        return math.sin((step/warmup_end) * (math.pi/2.))
    else:
        return 1.
